...
class Server:
    def __init__(self):
        ...
        self.max_current = rospy.get_param('~max_current', 0.0)  
        self.min_current = rospy.get_param('~min_current', 0.0)
        self.max_density = rospy.get_param('~max_density', 1028.0)
        self.min_density = rospy.get_param('~min_density', 1028.0)
        self.max_damping = rospy.get_param('~max_damping', 1.0)  
        self.min_damping = rospy.get_param('~min_damping', 1.0)
        self.max_weight  = rospy.get_param('~max_weight',  0.0)
        self.min_weight  = rospy.get_param('~min_weight',  0.0)
        self.max_current -= self.min_current
        self.max_density -= self.min_density
        self.max_damping -= self.min_damping
    ...
    def _onPost(self):
        ...
        for i in range(req['repeat']+1):
            ...
            self.spawnObject(self.usv_spawn_req, usv)
            # Set Domain Randomization (if any)
            self.setCurrent(t,pdf)
            self.setDamping(t,pdf)
            self.setWaterDensity(t,pdf)
            self.setWeight(t,pdf)
    ...
    def setCurrent(self, t, p):
        # Sample current velocity based on PDF
        velocity = np.random.choice(t*self.max_current + self.min_current, 1, p=p)[0]
        horizontal_angle = np.random.rand(1)[0]*2*np.pi - np.pi
        try:
    ...
    def setWaterDensity(self, t, p):
        # Sample water density based on PDF
        desired_density = np.random.choice(t*self.max_density + self.min_density ,1, p=p)[0]
        try:
            set_density = rospy.ServiceProxy(self.density_service_, SetFloat)
            resp = set_density(desired_density)
            print('Updated density with: '+str(desired_density))
        except rospy.ServiceException as e:
            print("Service call failed: %s" % e)

    def setDamping(self, t, p):
        # Sample water density based on PDF
        desired_density = np.random.choice(t*self.max_damping + self.min_damping ,1, p=p)[0]
        try:
            set_density = rospy.ServiceProxy(self.damping_service_, SetFloat)
            resp = set_density(desired_density)
            print('Updated damping with: '+str(desired_density))
        except rospy.ServiceException as e:
            print("Service call failed: %s" % e)

    def setWeight(self, t, p):
        # Sample water density based on PDF
        desired_density = np.random.choice(t*self.max_weight + self.min_weight ,1, p=p)[0]
        try:
            set_density = rospy.ServiceProxy(self.weight_service_, SetFloat)
            resp = set_density(desired_density)
            print('Updated weight with: '+str(desired_density))
        except rospy.ServiceException as e:
            print("Service call failed: %s" % e)
