...
class DreamerAgent:
    def __init__(self):
        ...
        self.obs['intensity'] = np.zeros((1,256,1),dtype=np.float32)
    ...
    def initialize_agent(self):
        ...
        for key, value in laser_int_policy.define_config().items():
        ...
        self.agent = laser_int_policy.Dreamer(config, actspace)
    ...
    def imageCallback(self, obs):
        ...
        if not self.Done:
            ...
            self.obs['intensity'][0] = self.intensity
            ...
            # Record
            self.incrementEpisode(self.obs['laser'][0], self.obs['image'][0], self.actions, self.obs['reward'][0], self.obs['physics'][0], self.obs['intensity'][0])
    ...
    def laserCallback(self, obs):
        ...
        self.intensity = np.expand_dims(np.max(np.reshape(np.array(obs.intensities),[-1,2]),axis=1)[-256:], -1)/200
