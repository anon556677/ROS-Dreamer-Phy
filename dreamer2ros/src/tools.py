...
class Module(tf.Module):
  ...
  def load(self, filename):
    ...
    #tf.nest.map_structure(lambda x, y: x.assign(y), self.variables, None)
...
def save_episodes(directory, episodes):
  ...
  for episode in episodes:
    ...
    filename = directory / str(timestamp)+'-'+str(identifier)+'-'+str(length)+'.npz'
  ...
  while True:
    for filename in directory.glob('*.npz'):
      if filename not in cache:
        try:
          ...
          print('Could not load episode: '+str(e))
          continue
    ...
    for index in random.choice(len(keys), rescan):
      ...
      if length:
        ...
        if available < 1:
          print('Skipped short episode of length '+str(available)+'.')
          continue
        if balance:
...
class Adam(tf.Module):
  ...
  def __call__(self, tape, loss):
    if self._variables is None:
      ...
      print('Found'+str(count)+' '+str(self._name)+' parameters.')
