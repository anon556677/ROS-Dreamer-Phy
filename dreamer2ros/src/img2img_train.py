...
class DreamerAgent:
    ...
    def ImageCallback(self, obs):
        self.image = np.reshape(np.fromstring(obs.data, np.uint8),[64,64,3])
     	if not self.Done:
            ...
            self.incrementEpisode(self.image, self.actions, self.obs['reward'][0], self.obs['physics'][0])
    ...
    def incrementEpisode(self, img, act, reward, phy):
    ...
    def saveEpisode(self):
        if self.training_mode:
            directory = pathlib.Path(self.save_directory+'/episodes')#.expanduser()
            try:
                directory.mkdir(parents=True)
            except:
                pass
        else:
            directory = pathlib.Path(self.save_directory+'/test_episodes')#.expanduser()
            try:
                directory.mkdir(parents=True)
            except:
                pass
        self.episode['physics'] = np.array(self.episode['physics'])
        self.episode['reward'] = np.array(self.episode['reward'])
        self.episode['action'] = np.array(self.episode['action'])
        self.episode['image'] = np.array(self.episode['image'])
